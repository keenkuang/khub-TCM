...
从 v0.2.5 到 v0.9.4，历经 **34 个版本迭代**，覆盖从单用户文档工具到多租户 AI 平台的完整跨越。1.0.0 标志着 API 稳定、文档完整的正式发布。

### 架构总结

```
数据底座 → 业务模块 → 多用户平台 → 临床智能 → 知识图谱推理
→ 私有化部署 → 开放平台 → AI Copilot → 工作流引擎
→ 统一搜索 → 离线同步 → 安全合规 → 数据分析
→ 集成中心 → 国际化 → AI Agent → 多租户 SaaS
→ 远程医疗 → 知识社区
```

### 1.0.0 新增
- Agent 定时调度器（`khub/agents/scheduler.py`）
- 全部 31 版本 spec + 29 版本 plan 文档完整
- 核心架构文档更新至 v1.0.0（architecture.md / config.md / api.md / deployment.md）
- 所有代码审查问题（Critical 2 + Important 6 + Minor 13）已修复
