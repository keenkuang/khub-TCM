...
- **低 · L1 触发器列枚举随 schema 漂移（§6.1）**：新增列不更新触发器即静默漏记，须代码生成触发器。**已修订：`install_triggers` 改为先 `DROP TRIGGER` 再 `CREATE`（原 `IF NOT EXISTS` 不会更新旧触发器），保证每次（重）安装按最新 `PRAGMA table_info` 重建；新增 `test_trigger_regenerates_on_schema_change` 回归。**
- **低 · L2 分区双写被包装成"已知风险"（§4.5）**：实为潜在损坏，应明示并以 PITR 为唯一兜底。**已修订：`disaster_recovery.md` §4.5 已明示"分区期双写 = 潜在数据损坏"，去掉"已知窗口/已知风险"淡化表述，明确 PITR 是唯一兜底。**
