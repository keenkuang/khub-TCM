...
- **当前阶段（M1–M5）不做多用户/权限系统**（纯个人使用）。
- 第一版不做语义检索（M4 再做，但数据模型预留）。
- 不做复杂的冲突自动合并（保留多版本，人工裁决）。
- 不绑定特定笔记 App 作为唯一规范存储（中枢自身即规范存储）。

### 团队协作（未来展望，不在本期范围）
当前为本地优先的个人中枢。若未来需要团队协作，属于一次较大的架构演进，至少需：
- 将中枢从「本地文件 + CLI」升级为**常驻服务**（多用户并发访问）；
- 引入**身份认证与权限/空间**；
- 并发写冲突处理从「多版本人工裁决」升级为更细的合并策略（必要时 CRDT/OT）；
- 同步引擎支持服务端统一编排而非各客户端各自拉取。
本期设计通过「适配器 + 版本化内核 + 界面解耦」已为上述留有余地（如未来加一层 hub-server 模式），但**不在此规格内实现**，也不阻塞 M1–M5。

### LLM 原生知识层 / "LLM wiki"（未来展望）
若未来引入 LLM wiki 类理念（知识以 LLM 易消费的方式组织：语义化、自动关联/反向链接、可被模型直接检索与问答、生成 `llms.txt`/上下文包等），当前设计已预留接口空间，无需改动内核契约：
- **`embeddings` 表已预留**（M4 填向量），是 LLM wiki 语义层的基础。
- **摄入管线可扩展产出"wiki 产物"**：未来每个文档除原文/版本外，可额外生成结构化摘要、反向链接、标签、上下文包，作为可选产物存储（新增表或 `document_versions.note` 扩展），不破坏现有字段。
- **查询层可返回 LLM-ready 上下文包**：M4 语义检索之上可叠加一层，将命中文档+摘要+链接组装成模型可直接消费的上下文，供 RAG/问答使用。
- **适配器契约稳定**：新增"wiki 产物生成器"作为管线插件，已有适配器与 Sync 引擎无需改动。
同样**不在此规格内实现**，仅确认架构为其留位，不阻塞 M1–M5。

### LLM wiki（未来展望，预留接口空间）
可借鉴「LLM 维护的活体知识库（LLM wiki）」理念：由本地模型持续阅读中枢文档，自动生成摘要、索引、跨文档链接、推断标签，形成一层「派生的、可追问」的知识视图。

为未来支持该模式，本期预留以下接口空间（不实现）：
- `document_versions.origin` 预留取值 `llm`（区别于 `hub` / `source`），用于标记**模型生成的派生内容**（摘要/索引/链接），避免被误当作原始文档写回源。
- `documents` 预留 `doc_type`（raw / derived），区分原始入库文档与 LLM 派生文档。
- M4 的 `embeddings` 层是该模式的检索基底；未来可加一个 `llm-maintainer` 角色（或适配器），只读中枢、只写 `origin=llm` 的派生版本。
- Sync 引擎需对 `origin=llm` 的文档**默认不向任何源 push**（派生内容不回流上游），此规则在引擎中预留开关。

该能力与团队协作一样属于远期演进，不进入 M1–M5 实现，但上述字段与规则已为其在数据模型与引擎层面留好位置。
