...
- **分区期双写 = 潜在数据损坏（非"可接受风险"、非"已知窗口"轻描淡写，PITR 是唯一兜底）**：两节点链路全断且各自存活时，旧主在分区期内**持续对外写**，与新主形成两条独立写入时间线（各自 epoch），冲突行**无法自动合并**——这是**潜在数据损坏**，任何"自愈/双向同步"都是幻觉。fencing 只保护"重连后"瞬间，**拦不住分区窗口内的双写**；该风险须明示其破坏力、不得包装成"已知风险"淡化。唯一数据兜底手段是 **PITR**：保留全量 WAL + 多版本快照，重连后先 `khub ha reconcile` 比对分歧、进 safe_mode 由人工决策，再经快照 + `replay_from(lsn<=target)` 回放到一致点；**没有 PITR 则分区双写不可恢复**。
