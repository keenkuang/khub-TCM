...
-- 正文文本独立表，供 FTS5 external-content 引用（正文只存一份，未来迁移用）
...
-- docs_fts：当前实现用 trigram 分词器以支持中文子串检索；
-- 后续可迁移为 external content（content='doc_text'）避免正文双份存储
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| M1 | SQLite + FTS5 + 版本化 | ✅ 完成 |
| M2 | PDF/EPUB 入库/存储（不入库闭环 + 入库全文索引） | ✅ 完成 |
| M3 | 全文检索 + 向量检索（暴力版，离线 LocalEmbedder） | ✅ 完成（`khub/retrieval.py`） |
| M4 | `LLMProvider` 抽象 + NoOp 占位（真实实现待接入） | ✅ 骨架完成 |
| M5 | 中医考试培训系统：题库 CRUD + LLM 出题/判分占位 | ✅ 骨架完成（`khub/exam`） |
| M6 | **患者数字孪生体管理系统**：病历 + 问诊 + 孪生体汇总 | ✅ 骨架完成（`khub/clinical`） |
| M7 | 门诊运营管理系统：排班/预约/就诊 | ✅ 骨架完成（`khub/ops`） |
| M8 | 向量检索升级 ANN / 真实 LLM 接入 / 封面提取 | ⏳ 待做 |

> 所有上层模块（考试 / 患者数字孪生 / 门诊运营）的共同地基：`document_versions` + `embeddings` + `docs_fts` + `LLMProvider` + 受管库。它们只新增自己的表，不修改核心库表。
...
- 已实现：薄 REST 层 `khub/api.py`（标准库 `http.server`，零依赖），直接调用核心函数；端点：`GET /ebooks`、`POST /ebooks/register`、`POST /ebooks/{cid}/ingest`、`GET /search?q=`。
- 原则：业务逻辑与传输层解耦，先有稳定库 API，HTTP 按需叠加。

## 五、安全（数据安全 / 网络安全）

从一开始就定好**默认安全**的取向，但不提前造轮子：

### 5.1 网络安全
- REST API 默认绑定 `127.0.0.1`（不暴露到公网），由 `khub serve --host/--port` 控制。
- 当前为本地开发，无认证；一旦需要远程访问，**只在 API 层加认证**（API Key / Token），不动核心库。架构已为此留白。
- FTS 查询用参数化 `MATCH ?`，无 SQL 注入；非法查询已兜底返回空结果（不抛 500）。

### 5.2 数据安全
- 数据均为本地文件：SQLite 库 + 受管库目录。默认靠文件系统权限保护。
- 涉及病历/问诊（PII）时再引入：加密落盘（SQLCipher 或文件系统加密）、访问审计、最小权限。
- 备份 = 复制 `~/.khub/` 整个目录（DB + 受管库），因二进制在磁盘，备份简单完整。
- 去重靠 sha256，避免同一文件多份存储导致的冗余与泄漏面扩大。

## 六、临床系统：患者数字孪生体管理系统 + 门诊运营管理系统

khub 不止是知识库，还需承载临床业务系统。它们都建立在 §五 的安全地基之上，并复用同一套检索/LLM 能力。

### 6.1 患者数字孪生体管理系统（Patient Digital Twin）

把"病历系统 + 问诊系统"统一抽象为**患者数字孪生体**——以患者为中心，聚合其全部医疗数据，形成可随诊次更新的数字模型。

- 顶层模块：`患者数字孪生体管理系统`，下辖**可扩展**的子系统：
  - **病历子系统**：结构化/非结构化病历（`records` 表），含诊断、处方、检验等。
  - **问诊子系统**：问诊记录与辨证（`consultations` 表），含主诉、舌脉、辨证、方案。
  - **未来子系统**：如随访、量表评估、可穿戴数据接入等，按同一模式挂接。
- 核心实体建议新增 `patients`（孪生体注册表：患者标识、基本信息、孪生版本），`records` / `consultations` 均通过 `patient_id` 关联；同一患者的多次诊次形成时间线，天然对应 khub 已有的**版本化**能力。
- 孪生体本身可由 LLM 基于 records+consultations 汇总生成"当前状态摘要"，存入 `document_versions`（doc_type='twin'），随新数据到来重新生成——复用现有版本与 FTS。

### 6.2 门诊运营管理系统

独立顶层模块，管理诊所日常运营，与临床内容解耦：

- 排班、预约、就诊队列、收费/医嘱流等（`schedules` / `appointments` / `visits` 等表）。
- 与孪生体系统通过 `patient_id`、`visit_id` 关联，但不共用业务字段，互不改核心表。
- 同样只新增自己的表，复用已建的 `LLMProvider`、检索与受管库。

### 6.3 设计约束（沿用）

- 所有临床/运营模块**只新增独立表与独立包**（如 `khub/clinical/twin.py`、`khub/clinical/records.py`、`khub/ops/`），核心库表（`documents`/`document_versions`/`embeddings`/`docs_fts`/`files`）保持稳定。
- 涉及 PII（病历、孪生体）时启用 §五 的加密落盘 + 访问审计。
- 对外仍走 §四 的薄 REST 层，按模块加路由前缀（如 `/twin`、`/ops`）。
